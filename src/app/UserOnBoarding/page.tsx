...
export default function UserOnboarding() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mobile_numbers.some((number) => !mobileRegex.test(number))) {
      setMessage({
        type: "error",
        content: "Please provide valid mobile numbers (maximum 25 digits, numbers only).",
      });
      setShowUpdateConfirmation(false);
      return;
    }
  ...
  const handleConfirmation = async () => {
    ...
    try {
      ...
      if(error.response.data.status==409 && error.response.data.message.includes("Email already exist")){
        setMessage({
          type: "error",
      ...
      if(error.response.data.status==409 && error.response.data.message.includes("Mobile number already exist.")){
        setMessage({
          type: "error",
